...
inline void timer::check(const char* msg)
{
  ...
  std::cout << int(acc_time + (running ? elapsed_time() : 0)) / (60 * 60 * 24) ;
  std::cout << " d  ";
  std::cout << int(acc_time + (running ? elapsed_time() : 0)) / (60 * 60) % 24;
  std::cout << " h  ";
  std::cout << int(acc_time + (running ? elapsed_time() : 0)) / 60  % 60 ;
  std::cout << " m  ";
  std::cout << int(acc_time + (running ? elapsed_time() : 0)) % 60;
  std::cout << " s        [" << acc_time + (running ? elapsed_time() : 0) << " sec.]" <<std::endl;
...
{
  os << t.acc_time + (t.running ? t.elapsed_time() : 0);
  return os;
}
